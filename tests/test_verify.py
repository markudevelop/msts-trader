...
def _order(ticker, side, dollars):
    return Order(
        ...
        side=side,
        quantity=Decimal("1"),
        estimated_price=Decimal(str(dollars)),
        notional=Decimal(str(dollars)),
    )


def test_converged_when_no_residual_orders():
    # every leg within drift -> no orders -> converged
    res = check_convergence(_preview([_row("SPY", note="within drift"), _row("IWM", note="within drift")]))
...
def test_residual_buy_flags_not_converged():
    ...
    rows = [_row("SPY", note="within drift"), _row("IWM", order=_order("IWM", Side.BUY, 8000))]
...
def test_residual_dollars_sums_abs():
    rows = [_row("A", order=_order("A", Side.BUY, 5000)), _row("B", order=_order("B", Side.SELL, 3000))]
    res = check_convergence(_preview(rows))
    assert res.residual_dollars == Decimal("8000")
    assert res.ok is False
...
def test_sell_proceeds_fund_the_buy():
    ...
    rows = [_row("XLP", order=_order("XLP", Side.SELL, 10000)), _row("IWM", order=_order("IWM", Side.BUY, 8000))]

...
def test_balances():
    bal = _broker(
        balances=SimpleNamespace(
            net_liquidating_value="42000",
            cash_balance="-29000",
            equity_buying_power="6000",
            derivative_buying_power="3000",
        )
    ).balances()
...
def test_place_market_enum_status_normalized_to_value():
    """The SDK returns an OrderStatus enum; str() yields 'OrderStatus.REJECTED'
    and defeats downstream 'rejected' checks — report the enum's .value."""
    import enum

    class St(enum.Enum):
        REJECTED = "Rejected"

    resp = SimpleNamespace(order=SimpleNamespace(id=9, status=St.REJECTED))
    b = _broker(positions=[], place_resp=resp)
    r = b.place_market(Order(ticker="SPY", side=Side.BUY, quantity=Decimal("10")))
...
def test_place_limit_enum_status_normalized_to_value():
    ...
    resp = SimpleNamespace(order=SimpleNamespace(id=9, status=St.LIVE))
    b = _broker(positions=[], place_resp=resp)
    r = b.place_limit(Order(ticker="SPY", side=Side.BUY, quantity=Decimal("10")), Decimal("100"))
